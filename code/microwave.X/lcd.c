...
void LCD_init(void)
{
	CONTROLL_PORT_DIR &=~(1<<RS);
    CONTROLL_PORT_DIR &=~(1<<RW);
    CONTROLL_PORT_DIR &=~(1<<E);

#if (DATA_BITS_MODE == 4)
#ifdef UPPER_PORT_PINS
	DATA_PORT_DIR &= 0x0F; /* Configure the highest 4 bits of the data port as output pins */
#else
	DATA_PORT_DIR &= 0xF0; /* Configure the lowest 4 bits of the data port as output pins */
#endif     //end if for #IFDEF
	LCD_sendCommand(FOUR_BITS_DATA_MODE); /* initialize LCD in 4-bit mode */
	LCD_sendCommand(TWO_LINE_LCD_Four_BIT_MODE); /* use 2-line lcd + 4-bit Data Mode + 5*7 dot display Mode */
#elif (DATA_BITS_MODE == 8)
	DATA_PORT_DIR = 0x00; /* Configure the data port as output port */
